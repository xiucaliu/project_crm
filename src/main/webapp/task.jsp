                                    ...
                                    <c:forEach var="listObj" items="${list}">
                                        <tr>
                                            ...
                                            <c:forEach var="jobListObj" items="${jobList}">
                                                ...
                                                <c:if test = "${listObj.job_id == jobListObj.id }">
                                                    <td><c:out value="${jobListObj.name}"/></td>
                                                </c:if>
                                            </c:forEach>

                                            <c:forEach var="userListObj" items="${userList}">
                                                <c:if test = "${listObj.user_id ==userListObj.id }">
                                                    <td><c:out value="${userListObj.fullname}"/></td>
                                                </c:if>
                                            </c:forEach>
                                            <td><c:out value="${listObj.getStart_date()}"/></td>
                                            <td><c:out value="${listObj.getEnd_date()}"/></td>
                                            <td><c:out value="${listObj.status_id}"/></td>
                                            <td>
                                                <a href=<c:url value = "/task/update?id=${listObj.id}"/> class="btn btn-sm btn-primary">Sửa</a>
                                                <span taskid = "${listObj.id}" class="btn btn-sm btn-danger btn-delete-task">Xóa</span>
                                                <a href=<c:url value = "/task/details?id=${listObj.id}"/> class="btn btn-sm btn-info">Xem</a>
                                            </td>
                                        </tr>
                                    </c:forEach>
                                    </tbody>
