                            ...
                            <table class="table" id="example">
                                ...
                                <c:forEach var="listObj" items="${list}">
                                    <tr>
                                        ...
                                        <c:forEach var="leaderListObj" items="${leaderList}">
                                            <c:if test="${listObj.leaderId == leaderListObj.id}">
                                                <td><c:out value="${leaderListObj.fullname}"/></td>
                                            </c:if>
                                        </c:forEach>
                                        <td><c:out value="${listObj.startDate}"/></td>
                                        <td><c:out value="${listObj.endDate}"/></td>
                                        <td>
                                            <a href="<c:url value = "/job/update?id=${listObj.id}" />"
                                               class="btn btn-sm btn-primary">Sửa</a>
                                            <span jobid="${listObj.id}"
                                                  class="btn btn-sm btn-danger btn-delete-job">Xóa</span>
                                            <a href="<c:url value = "/job/details?id=${listObj.id}"/>"
